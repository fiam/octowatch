...
Items in `Focus`, `Notifications`, and `Workflows` have dot-style read/unread indicators, and opening one of those items marks it as read locally. The dedicated `Pull Requests` and `Issues` dashboards do not use local read state.
The main window sidebar supports native macOS multi-selection, and a right-click menu can open, ignore, and, where read state is supported, mark the current selection read or unread.
